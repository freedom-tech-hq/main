...
import type { CollectionLikeId, MailCollectionGroup } from 'freedom-email-user';
...
export const useMailCollectionsListDataSource = (): DataSource<MailCollectionsListCollectionDataSourceItem, CollectionLikeId> => {
  const tasks = useTasks();

  const groups = useRef<MailCollectionGroup[]>([]);
  const items = useRef<MailCollectionsListCollectionDataSourceItem[]>([]);
