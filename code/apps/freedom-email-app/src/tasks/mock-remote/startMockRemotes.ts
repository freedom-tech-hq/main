...
export interface MockRemotes {
  ...
  readonly getMockRemoteStore: PRFunc<InMemorySyncableStore, 'not-found', [remoteId: RemoteId, storageRootId: StorageRootId]>;
...
export const startMockRemotes = async (trace: Trace, { creatorPublicCryptoKeysSet }: WithMockRemotesArgs): PR<MockRemotes> => {
  ...
  const userStores: Partial<Record<`${RemoteId}.${StorageRootId}`, InMemorySyncableStore>> = {};
  ...
  const onFolderAddedOrRemoved = makeAsyncResultFunc(
    ...
    async (trace, args: { store: SyncableStore; remoteId: RemoteId; path: StaticSyncablePath }) => {
      ...
      return triggerContentChangeNotificationSoonForPath(trace, { ...args, path: parentFolderPath.value });
    }
  );
  ...
  const onNeedsSync = makeAsyncResultFunc(
    ...
    async (trace, args: { store: SyncableStore; remoteId: RemoteId; path: StaticSyncablePath }) => {
      ...
      return triggerContentChangeNotificationSoonForPath(trace, { ...args, path: folderPath.value });
    }
  );
  ...
  const getMockRemoteStore = makeAsyncResultFunc(
    ...
    async (trace: Trace, remoteId: RemoteId, storageRootId: StorageRootId): PR<InMemorySyncableStore, 'not-found'> => {
  ...
  const createMockRemoteStore = makeAsyncResultFunc(
    ...
    async (
    ...
    ): PR<InMemorySyncableStore, 'conflict'> => {
      const key: `${RemoteId}.${StorageRootId}` = `${remoteId}.${storageRootId}`;
      ...
      const newStore = new InMemorySyncableStore({ storageRootId, provenance, cryptoService });
      store = newStore;
  ...
  const puller: SyncPuller = makeAsyncResultFunc(
    ...
    async (localTrace, { remoteId, path, hash, sendData = false }): PR<SyncPullResponse, 'not-found'> => {
      ...
      switch (remoteItemAccessor.value.type) {
        case 'folder':
          return pullFolder(trace, { store: store.value, path, hash });

        case 'flatFile':
          return pullFlatFile(trace, { store: store.value, path, hash, sendData });

        case 'bundleFile':
          return pullBundleFile(trace, { store: store.value, path, hash });
      }
    }
  );
  ...
  const pusher: SyncPusher = makeAsyncResultFunc(
    ...
    async (localTrace, { remoteId, type, path, data, provenance }): PR<undefined> => {
      ...
      let store: Result<InMemorySyncableStore, 'conflict' | 'not-found'> = await disableLam(trace, 'not-found', (trace) =>
      ...
      switch (type) {
        ...
        case 'bundleFile': {
          const bundle = await createViaSyncBundleFileAtPath(trace, store.value, path, provenance);
          if (!bundle.ok) {
...
const pullBundleFile = makeAsyncResultFunc(
  [import.meta.filename, 'pullBundleFile'],
  async (
    trace: Trace,
    { store, hash, path }: { store: SyncableStore; path: StaticSyncablePath; hash: Sha256Hash | undefined }
  ): PR<InSyncBundleFile | OutOfSyncBundleFile, 'not-found'> => {
    const bundle = await getSyncableAtPath(trace, store, path, 'bundleFile');
    if (!bundle.ok) {
      if (bundle.value.errorCode === 'deleted') {
    ...
    if (remoteHash.value === hash) {
      return makeSuccess({ type: 'bundleFile', outOfSync: false } satisfies InSyncBundleFile);
    }
...
const pullFlatFile = makeAsyncResultFunc(
  [import.meta.filename, 'pullFlatFile'],
  async (
    trace: Trace,
    { store, hash, path, sendData }: { store: SyncableStore; path: StaticSyncablePath; hash: Sha256Hash | undefined; sendData: boolean }
  ): PR<InSyncFlatFile | OutOfSyncFlatFile, 'not-found'> => {
    ...
    if (remoteHash.value === hash) {
      return makeSuccess({ type: 'flatFile', outOfSync: false } satisfies InSyncFlatFile);
    }
    ...
    if (!sendData) {
      return makeSuccess({ type: 'flatFile', outOfSync: true, provenance: remoteProvenance.value } satisfies OutOfSyncFlatFile);
    } else {
