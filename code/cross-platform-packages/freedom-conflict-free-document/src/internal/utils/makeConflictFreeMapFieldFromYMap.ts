...
import type { ConflictFreeMapField } from '../../types/ConflictFreeMapField.ts';
...
export const makeConflictFreeMapFieldFromYMap = <KeyT extends string, ValueT>(
  yDoc: Y.Doc,
  fieldInfos: ConflictFreeDocumentFieldInfos,
  fieldName: string,
  schema: Schema<ValueT>
): ConflictFreeMapField<KeyT, ValueT> => {
  ...
  const field: ConflictFreeMapField<KeyT, ValueT> = {
    ...
    entries: (range, reverse = false): IterableIterator<[KeyT, ValueT]> => {
      const yMap = getNativeField({ create: false });
      /* node:coverage disable */
      if (yMap === undefined) {
        const emptyIterator: IterableIterator<[KeyT, ValueT]> = {
      ...
      const iterator: IterableIterator<[KeyT, ValueT]> = {
        ...
        next: () => {
          ...
          const value: [KeyT, ValueT] = [cursorKey, field.get(cursorKey)!];
          cursor += step;
