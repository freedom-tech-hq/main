...
export const createInitialSyncableStoreStructureForUser = makeAsyncResultFunc(
  ...
  async (trace, access: EmailAccess): PR<undefined> => {
    ...
    if (!mailStorageBundle.ok) {
      return generalizeFailureResult(trace, mailStorageBundle, ['conflict', 'not-found', 'untrusted', 'wrong-type']);
    }

    // Mail Out Folder
    const mailOutFolder = await createFolderAtPath(trace, userFs, paths.out.value);
    if (!mailOutFolder.ok) {
      return generalizeFailureResult(trace, mailOutFolder, ['conflict', 'not-found', 'untrusted', 'wrong-type']);
    }

    // Mail Drafts Bundle
    const mailDraftsBundle = await createBundleAtPath(trace, userFs, paths.drafts.value);
    if (!mailDraftsBundle.ok) {
      return generalizeFailureResult(trace, mailDraftsBundle, ['conflict', 'not-found', 'untrusted', 'wrong-type']);
    }

    // Mail Indexes Bundle
    const mailIndexesBundle = await createBundleAtPath(trace, userFs, paths.indexes.value);
    if (!mailIndexesBundle.ok) {
      return generalizeFailureResult(trace, mailIndexesBundle, ['conflict', 'not-found', 'untrusted', 'wrong-type']);
    }
    ...
    if (!mailIdsByMessageIdIndexBundle.ok) {
      return generalizeFailureResult(trace, mailIdsByMessageIdIndexBundle, ['conflict', 'not-found', 'untrusted', 'wrong-type']);
    }

    // Mail Threads Bundle
    const mailThreadsBundle = await createBundleAtPath(trace, userFs, paths.threads);
    if (!mailThreadsBundle.ok) {
      return generalizeFailureResult(trace, mailThreadsBundle, ['conflict', 'not-found', 'untrusted', 'wrong-type']);
    }
