...
// Load env settings
const rootDir = import.meta.dirname.endsWith('/lib/mjs')
  ? `${import.meta.dirname}/../..`
  : `${import.meta.dirname}/..`;
...
export const SMTP_PORTS = env.get('SMTP_PORTS').required().asCustom(
  value => value.split(',').map(port => {
...
export const SMTP_TLS_CERT_RAW =
  ...
  env.get('SMTP_TLS_CERT').required().asCustom(
    value => fs.readFileSync(path.resolve(rootDir, value), 'utf8')
  ) as string;

/** TLS key path for SMTP server */
export const SMTP_TLS_KEY_RAW =
  env.get('SMTP_TLS_KEY_RAW').asString() ||
  env.get('SMTP_TLS_KEY').required().asCustom(
    value => fs.readFileSync(path.resolve(rootDir, value), 'utf8')
  ) as string;
...
export const SMTP_OUR_DOMAINS = env.get('SMTP_OUR_DOMAINS').required().asCustom(
...
export const GOOGLE_APPLICATION_CREDENTIALS_RAW =
  env.get('GOOGLE_APPLICATION_CREDENTIALS_RAW').asCustom((value) => (value
    ? JSON.parse(value)
    : undefined
  )) as {} | undefined ||
  env.get('GOOGLE_APPLICATION_CREDENTIALS')
    .asCustom((value) => (value
      ? JSON.parse(fs.readFileSync(path.resolve(rootDir, value), 'utf8'))
